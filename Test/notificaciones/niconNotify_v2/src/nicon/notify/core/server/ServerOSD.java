...
//            if(stackServer.size()<=4){
            ...
            if(Notification.SOLO_UNO){
                // acá tengo que recorrer todas las notificacione sy preguntar si es
                // distinta a la notificacion actual, porque si es asi
                // tengo que visible false
            }
...
//            }
    ...
    public void remove(int nid) {
        System.out.println("Intentando borrar NID: " + nid);
        ServerThread thrd;
        if (stackServer.containsKey(nid)) {
            System.out.println("Stack server contiene el NID!");
            notify = (DesktopNotify) stackServer.get(nid);
            if (notify.isVisible()) {
                notify.dispose();
            }
            
            stackServer.remove(nid);System.out.println("3");
            stackThread.remove(nid);System.out.println("4");
            System.out.println("ServerOSD: notification with nid: " + nid + " remove successfully ...");
            thrd = (ServerThread) stackThread.get(nid);System.out.println("1");
//            thrd.killThread();System.out.println("2");
        }
    }
    ...
    private int setVerticalPosition() {
        if (stackServer.size() == 1 || Notification.SOLO_UNO) {
            yPosition = 30;
