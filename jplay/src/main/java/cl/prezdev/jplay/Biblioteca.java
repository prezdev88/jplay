...
public class Biblioteca implements Serializable{
    ...
    public void addFavorita(Song c){
        favoritos.add(c);
        Log.add("Añadida a favoritos: "+c);
        msFavoritos += c.getMilisDuration();
    }
    
    public void removeFavorita(Song c){
        favoritos.remove(c);
        Log.add("Removido de favoritos: "+c);
        msFavoritos -= c.getMilisDuration();
    }
    
    public boolean isFavorita(Song c){
        return favoritos.contains(c);
    }

    public List<Song> getFavoritos() {
        return favoritos;
    }
    ...
    public void add(Song c) {
        if (!estaCancion(c)) {
            ...
            msBiblioteca += c.getMilisDuration();
        }
    }
    ...
    public boolean estaCancion(Song c) {
        return canciones.contains(c);
    }

    public List<Song> getCancionesMasReproducidas() {
        ...
        for (Song c : canciones) {
            if (c.getPlayCount() != 0) {
                topCanciones.add(c);
                msMasEscuchadas += c.getMilisDuration();
            }
        }
...
//        printAlbums();
    ...
    private void addToAlbum(Song c) {
        ...
        for (Album a : albums) {
            ...
            if (a.getName().trim().equalsIgnoreCase(c.getAlbum().trim())) {
                ...
                if (!a.songExist(c)) { // este if es por el issue #12
                    // la añado!
                    a.addSong(c);
                }
                encontrado = true;
        ...
        if (!encontrado) {
            Album a = new Album(c.getAuthor(), c.getAlbum(), c.getYear());
            a.addSong(c);
